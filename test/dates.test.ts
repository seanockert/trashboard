...
import { dateCandidates, penaltyCandidates, selected } from '../src/jev/candidates';
...
describe('dates in text', () => {
  ...
  it('gives a date with no year the year after publication when it is earlier', () =>
    expect(dateCandidates('Held on 9 May. Submissions close 3 January. On 15 April a notice.', '2026-04-20').map((c) => c.value)).toEqual(['2026-05-09', '2027-01-03', '2026-04-15']));
});
...
describe('Jev selections', () => {
  it('uses only a penalty from the text, and only when Jev is sure', () => {
    ...
    expect(selected({ candidates, answer: { choice: 'amount_1', confidence: 0.9 } })).toBe(60000);
    expect(selected({ candidates, answer: { choice: 'amount_1', confidence: 0.3 } })).toBeNull();
    expect(selected({ candidates, answer: { choice: 'none', confidence: 0.9 } })).toBeNull();
  });
  it('gives each date one candidate, and uses only a date from the text', () => {
    ...
    expect(candidates.map((c) => c.value)).toEqual(['2027-07-01', '2026-07-02']);
    expect(selected({ candidates, answer: { choice: 'date_2', confidence: 0.9 } })).toBe('2026-07-02');
    expect(selected({ candidates, answer: { choice: 'date_2', confidence: 0.4 } })).toBeNull();
  });
});
