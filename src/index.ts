...
import { handleItemMessage, IngestMessage, ingestPage, ItemMessage, retagStale, scheduleAll } from './pipeline';
...
export default {
  ...
  async scheduled(_controller, env, ctx) {
    ctx.waitUntil(Promise.all([scheduleAll(env), retagStale({ env, limit: RETAG_LIMIT, minAgeHours: 6 })]));
  },
  ...
  async queue(batch, env) {
    await Promise.all(
      batch.messages.map((message) => {
        ...
        if (batch.queue === ITEM_QUEUE) return handle({ message, queue: batch.queue, schema: ItemMessage, run: (body) => handleItemMessage({ env, message: body }) });
